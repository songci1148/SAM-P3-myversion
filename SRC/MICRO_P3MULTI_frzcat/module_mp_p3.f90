...
!BG add accumulation of microphysical process rates here [in main loop as rates are 1dim or in iice function]
 if (do_accumulate_micro_proc_rates) then
    ...
    idx = idx + 1
    !BGBGGBmicro_proc_rates(i,k,idx) = micro_proc_rates(i,k,idx) + SUM(qinuc(:)) ! deposition/condensation freezing nuc [kg/kg/s] ABS added but not sure what it is
    !GBGBGBidx = idx + 1
    !BG ADDED!!!
    micro_proc_rates(i,k,idx) = micro_proc_rates(i,k,idx) + SUM(ninuc2(:)) + SUM(ninuc3(:)) !freezing from Mohler (if enabled) or Liu and Penner/HOM frz
    !here currently end, BG
    idx = idx + 1
