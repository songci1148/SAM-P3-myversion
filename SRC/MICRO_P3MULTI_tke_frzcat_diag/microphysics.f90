...
!BG
!use micro_sgs_init, only:tke !BG
use vars, only: w2_sgs !BG
...
subroutine micro_init()
  ...
  if(freezing_3dout) then
    ...
    print*, 'nfields3D_micro',nfields3D_micro
    print*, 'nmicro_proc',nmicro_proc
 ! else
...
subroutine micro_proc()
   ...
   real :: tmpqc(nx, nzm), tmpnc(nx, nzm), tmpqr(nx, nzm), tmpnr(nx, nzm), &
           ...
           tmpwtke(nx,nzm) !BG added for subgrid TKE for freezing
   ...
   !no need for this ? real, intent(in), dimension(1:nx,1:ny,1:nzm) :: w2_sgs ! BG subgrid tke m2/s2
   ...
   do j = 1, ny
    ...
    do k = 1,nzm
      do i = 1, nx
        ...
        !BG subgrid scale w from tke, as per above for tmpw:
        tmpwtke(i,k) = sqrt((zi(k+1)-z(k))*w2_sgs(i,j,k)+ &
             (z(k)-zi(k))*w2_sgs(i,j,k+1))/(zi(k+1)-zi(k))
        !tmpwtke(i,k) = sqrt(10*w2_sgs(i,j,k)) !BG
      end do
    end do
     ...
     ! note: code for prediction of ssat not currently avaiable, set 2D array to 0
      ...
      call p3_main( qc=tmpqc, nc=tmpnc, qr=tmpqr, nr=tmpnr, &
              ...
              do_accumulate_micro_proc_rates=do_accumulate_micro_proc_rates,wtke=tmpwtke) !BG added
...
!bloss     cloudliq(1:nx,j,1:nzm) = tmpqc(1:nx,1:nzm)
     ...
     if(reff_ice_holds_Dge) then
       ...
       do ii = 1,nCat
         ...
         ReffIce_P3(1:nx,j,1:nzm,ii) = ReffIce_P3(1:nx,j,1:nzm,ii)*1.5396 ! ABS 11/21/24: fixed error in reff-to-Dge conversion 
...
!else
!  nfields1=nfields1+1 !BG quick fix, make it better later
!end if !!!!!
