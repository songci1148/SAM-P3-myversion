...
! Specify te tracers' default names:
 ...
 ntr =0
 ntr = ntr+1
 tracerlongname(ntr) = 'Hom freezing time tracer (set to 1 where homogeneous freezing occurs, elsewhere decays w/tau=2 hours)'
 ntr = ntr+1
...
! Set surface and top fluxes of tracers. Default is 0 set in setdata.f90
 ...
 ! The transport is done automatically. 
  ...
  do ntr = 1,ntracers
    ...
    select case (ntr)
    case (1)
      ! Tracer 1: Homogeneous freezing tracer (==1 if freezing in liquid phase, decays as 1/(2hours) elsewhere)
      ...
      !aaaa
        ...
        do j = 1,ny
          do i = 1,nx
            if ( (micro_proc_rates(i,j,k,36) ) .gt.1.e-8)  then !Homogeneous phase frz
             !if ( (w(i,j,k) + w(i,j,k+1)).gt.2.) then
              tracer(i,j,k,ntr) = 1.
            end if
          end do
        end do
    ...
    ! Tracer 2: In situ nucleation tracer
      ...
      do k = 1,nzm
        ...
        do j = 1,ny
          do i = 1,nx
            !if( (w(i,j,k) + w(i,j,k+1)).gt.2.) then
            if ( (micro_proc_rates(i,j,k,16)) .gt.1.e-8)  then
              tracer(i,j,k,ntr) = 1.
            end if
          end do
        end do
    ...
    case (3)
      ! Tracer 3: mixed phase freezing tracer
      ...
      do k = 1,nzm
        ...
        do j = 1,ny
          do i = 1,nx
            if ( (micro_proc_rates(i,j,k,15)) .gt.1.e-8)  then
            !if( (w(i,j,k) + w(i,j,k+1)).gt.2.) then
              tracer(i,j,k,ntr) = 1.
            end if
          end do
    ...
    !     do j = 1,ny
    !       do i = 1,nx
    !         if ( (micro_proc_rates(i,j,k,15) + micro_proc_rates(i,j,k,16) + micro_proc_rates(i,j,k,23)) .gt.1.e-8)  then
    !         !if( (w(i,j,k) + w(i,j,k+1)).gt.2.) then             
    !            tracer(i,j,k,ntr) = 1.
    !         end if
    !       end do
    ...
    case (4)
      ...
      do k = 1,nzm
        ...
        do j = 1,ny
          do i = 1,nx
            if( ( (w(i,j,k) + w(i,j,k+1)).gt. 3.2 ) .AND. (qcl(i,j,k)+qci(i,j,k).gt.1.e-5) ) then
