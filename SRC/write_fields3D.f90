...
use tracers !BG for tracer 3 output
implicit none
...
integer i,j,k,n,nfields,nfields1,ntr
real(4) tmp(nx,ny,nzm)
integer, external :: lenstr

nfields=8 + nfields3D_micro +1 !+1! number of 3D fields to save
!nfields = 8 + nfields3D_micro + 2 !BG 2 from acresw+acrelw
...
if (acre3d) nfields=nfields+2 !BG acreLW + acreSW
if (csky3d) nfields=nfields+1 !BG clear sky field
if (trac3d) nfields=nfields+4 !tracer 1 5 !BG tracer 1,2,3
if (write_w2_sgs) nfields=nfields+1 !BG
nfields1=0
...
!!!!!!!!!!!!!!!!!!!!!BG W2_SGS!!!!!!!!!!!!!
if (write_w2_sgs) then

  nfields1=nfields1+1
  do k=1,nzm
   do j=1,ny
    do i=1,nx
      tmp(i,j,k)=w2_sgs(i,j,k)
    end do
   end do
  end do
  name='W2_SGS'
  long_name='subgrid-scale w2'
  units='m2/s2'
  call compress3D(tmp,nx,ny,nzm,name,long_name,units, &
                                 save3Dbin,dompi,rank,nsubdomains)
end if !write only if write_w2_sgs = true
!!!!!!!!!!!!!!!!!!!!!!!!!!BG END--------------------
...
if((dolongwave.or.doshortwave).and..not.doradhomo) then
 ...
 !BG added csky
  ...
  do k=1,nzm
   do j=1,ny
    do i=1,nx
      tmp(i,j,k)=qradcs(i,j,k)*86400.
    end do
   end do
  end do
  name='QRADC'
  long_name='Radiative heating rate: clear sky'
  units='K/day'
  call compress3D(tmp,nx,ny,nzm,name,long_name,units, &
                                 save3Dbin,dompi,rank,nsubdomains)
  end if !csky3d

  if (acre3d) then !BG added 1 jul 2019
   ...
   !BG added LW ACRE
    ...
    do k=1,nzm
     do j=1,ny
      do i=1,nx
        tmp(i,j,k)=acrelw(i,j,k)*86400.
      end do
     end do
    end do
    name='ACRELW'
    long_name='LW radiative heating rate: cloudy sky only'
    units='K/day'
    call compress3D(tmp,nx,ny,nzm,name,long_name,units, &
                                 save3Dbin,dompi,rank,nsubdomains)
   ...
   !BG added SW ACRE
    ...
    do k=1,nzm
     do j=1,ny
      do i=1,nx
        tmp(i,j,k)=acresw(i,j,k)*86400.
      end do
     end do
    end do
    name='ACRESW'
    long_name='SW radiative heating rate: cloudy sky only'
    units='K/day'
    call compress3D(tmp,nx,ny,nzm,name,long_name,units, &
                                 save3Dbin,dompi,rank,nsubdomains)
...
end if !if dolongwave or doshortwave and not doradhomo
...
if(trac3d) then !BG 3d output
 ...
 do ntr = 1,ntracers
    ...
    do k=1,nzm
     do j=1,ny
      do i=1,nx
        tmp(i,j,k)=tracer(i,j,k,ntr)!tr1(i,j,k)!
      end do
     end do
    end do
    name=tracername(ntr)
    long_name=TRIM(tracerlongname(ntr))
    units='1'
    call compress3D(tmp,nx,ny,nzm,name,long_name,units, &
                                 save3Dbin,dompi,rank,nsubdomains)
 end do
...
! END UW ADDITIONS
  ...
  if(masterproc) then
    ...
    if(RUN3D.or.save3Dsep) then
       ...
       !BG changed based on PB code (Jan 2024) systemf('gzip -f '//filename)
