...
!==========================================================================================!
 ...
 SUBROUTINE p3_main(qc,nc,qr,nr,th_old,th,qv_old,qv,dt,qitot,qirim,nitot,birim,ssat,uzpl, &
		    ...
		    micro_proc_rates,do_accumulate_micro_proc_rates,wtke) !BG/bloss/SAM: Add tendency output + BG added wtke
...
! note: Nc may be specified or predicted (set by log_predictNc)
 ...
 real, intent(in),    dimension(its:ite,kts:kte)      :: wtke       ! subgrid TKE vertical air velocity square        m s-1 !BG ADDED TEST
...
if (depo_frz_std) then !BG
  ...
  if ( ( (t(i,k).lt.258.15) .and. (t(i,k) .ge. 236.15) .and. (supi(i,k).ge.0.05) .and. (qc(i,k) .gt. qsmall) ) .or. ( (t(i,k).lt.241.15) .and. (t(i,k) .ge. 236.15) .and. (supi(i,k).ge.0.005) .and. (qc(i,k) .gt. qsmall) ) ) then  !BG added this to prevent too much "hom frz at -37"
   ...
   !BG aug 2023 test
   if (dust_ramp) then
       !temp_min = 203.15    ! Minimum temperature (K)
       !temp_max = 223.15    ! Maximum temperature (K)
       !value_min = 0.1      ! Corresponding value at minimum temperature
       !value_max = 1.0      ! Corresponding value at maximum temperature
       
       !if warmer than 233 K, use ndust = ndust * 1
       if (t(i,k) < 233.15) then !colder than -40
           ! Linear ramp / perform linear interpolation
           !value = value_min + (value_max - value_min) * (temp - temp_min) / (temp_max - temp_min)
           ndust = ndust* (ramp_min + (1 - ramp_min) * (t(i,k) - 203.15) / 30.0)
       else if (t(i,k)<203.15) then
           ndust = ndust * ramp_min
       end if
   end if !dust ramp BG 
  ...
  if (no_cirrus_mohler_ice_nucleation .eq. .false.) then
   ...
   if (do_new_lp_frz) then
     ...
     wbar2 = uzpl(i,k)
     if (do_wtke) then !BG!!!
            wbar1 = wbar1 + wtke(i,k)
            wbar2 = wbar2 + wtke(i,k)
     endif
     !===========================
    ...
    !based on Shi et al., 2015, ACP
       ...
       if (do_wtke) then !BG ADDED
           weff     = max(uzpl(i,k)+wtke(i,k)-wpice, minweff)
           wpice    = min(wpice, uzpl(i,k)+wtke(i,k))
           weffhet  = max(uzpl(i,k)+wtke(i,k)-wpicehet,minweff)
           wpicehet = min(wpicehet, uzpl(i,k)+wtke(i,k))
       else
   ...
   ni = 0.
    ...
    !print*,'WTKE in moduleP3=',wtke(i,k), 'w model',uzpl(i,k),'wtke/wmodel=',wtke(i,k)/uzpl(i,k)

            A = -1.4938 * log(ndust) + 12.884
            B = -10.41  * log(ndust) - 67.69

            regm = 0.8*(A * log(wbar1)) + B
           ...
           ! homogeneous nucleation only
               ...
               !BG HET freezing for HOM regime...where RHi not enough for HOM
               !if (nihf<1.) then
            ...
            ! transition between homogeneous and heterogeneous: interpolate in-between
               ...
               if (tc(i,k).lt.-40. .and. wbar1.gt.1.) then ! exclude T<-40 & W>1m/s from hetero. nucleation
                  ...
                  !BG HET freezing for transition regime...where RHi not enough for HOM
                  !if (nihf<1.) then
                  !    call hetero(regm,wbar2,ndust,niimm,nidep) !BG nidep is 0
                  !    n1 = niimm+nihf
                  !end if
