module rad
  ...
  use params, only : ggr, coszrs, cp, ocean,  &
       ...
       disablecloudradiation, disablecloudradiation_sw, acre3d !BG
	...
	! set trace gas concentrations.  Assumed to be uniform in the horizontal.
        ...
        do lat = 1, ny 
          ...
          if (dolongwave) then
            ...
            !BG 3D diagn field - clearsky
            qradcs(:, lat, :) = lwHeatingRateClearSky(:,1:nzm)

            !BG get 3D LW and SW acre !1 jul 2019
            if (acre3d) then
               acrelw(:, lat, :) = lwHeatingRate(:,1:nzm) - lwHeatingRateClearSky(:,1:nzm)
            end if

            !Bloss/Berry (May 7, 2015) +BG 2018: Make LW fluxes clear sky values if True
            if(disablecloudradiation) then
               qrad(:, lat, :) = lwHeatingRateClearSky(:, 1:nzm)
            !BG Sep 14, 2018: Make LW fluxes clear sky values if True for z>6.6 km (approx 440 hPa, high clouds)
            !else if (nolw_highcld) then
            !    qrad(:, lat, 44:nzm) = lwHeatingRateClearSky(:, 44:nzm)
            !    qrad(:, lat,  1:43)  = lwHeatingRate(:, 1:43)
            !else if (nolw_clearsky) then !BG 18jan019
            !    qrad(:,lat, :) = lwHeatingRate(:,1:nzm) - lwHeatingRateClearSky(:,1:nzm) !only cloudy part of heating rate
            else
              qrad(:, lat, :) = lwHeatingRate(:, 1:nzm) !BG
              !qrad(:, lat, :) = 0.2 * lwHeatingRate(:, 1:nzm) + 0.8*lwHeatingRateClearSky(:,1:nzm) !BG test 1/5 of LW ACRE only 
            end if
         ...
         if(doshortwave) then
            ...
            if(all(solarZenithAngleCos(:) >= tiny(solarZenithAngleCos))) then 
              ...
              !BG (May 7, 2018): Make SW fluxes clear sky values if True
              if(disablecloudradiation_SW) then
                qrad(:, lat, :) = qrad(:,lat,:) + swHeatingRateClearSky(:, 1:nzm)
              !else if (nosw_highcld) then !BG Sep 14,2018:Make SW fluxes clear sky values if True for high clouds (above 6.6 km, 440 hpa approx)
              !  qrad(:, lat, 44:nzm) = qrad(:,lat,44:nzm) + swHeatingRateClearSky(:, 44:nzm)
              !  qrad(:, lat,  1:43)  = qrad(:,lat, 1:43)  +swHeatingRate(:,1:43)
              !else if (nosw_clearsky) then !BG 18jan019
              !  qrad(:,lat, :) = qrad(:,lat,:)  + swHeatingRate(:,1:nzm) - swHeatingRateClearSky(:,1:nzm) !only cloudy part of heating rate
              else
                qrad(:, lat, :) = qrad(:,lat,:) + swHeatingRate(:,1:nzm)
              end if

              !BG 3D diagnostic - clearsky: = qradcs LW + qradcs SW 
              qradcs(:, lat, :) = qradcs(:,lat,:) + swHeatingRateClearSky(:,1:nzm)

              !BG get 3D LW and SW acre !1 jul 2019
              if (acre3d) then
                 acresw(:, lat, :) = swHeatingRate(:,1:nzm) - swHeatingRateClearSky(:,1:nzm)
              end if
