...
! Specify te tracers' default names:
 ...
 ntr =0
 ntr = ntr+1
 tracerlongname(ntr) = 'Ice nucleation time tracer (set to 1 where ice nucleation occurs, else decays w/tau=2 hours)'
 ntr = ntr+1
...
! Set surface and top fluxes of tracers. Default is 0 set in setdata.f90
 ...
 ! The transport is done automatically. 
  ...
  do ntr = 1,ntracers
    ...
    select case (ntr)
    case (1)
      ! Tracer 1: Freezing tracer (==1 if freezing, decays as 1/(2hours) elsewhere)
      ...
      do k = 1,nzm
        ...
        do j = 1,ny
          do i = 1,nx
            !NOW THESE DIFFERENT NUM OF MIC PROC RATE!! if ( (micro_proc_rates(i,j,k,15) + micro_proc_rates(i,j,k,16) + micro_proc_rates(i,j,k,23)) .gt.1.e-8)  then
            if ( (micro_proc_rates(i,j,k,1) + micro_proc_rates(i,j,k,2) + micro_proc_rates(i,j,k,3) + &
                 micro_proc_rates(i,j,k,4) + micro_proc_rates(i,j,k,5) + micro_proc_rates(i,j,k,10)) .gt.1.e-8)  then         
               tracer(i,j,k,ntr) = 1.
            end if
          end do
        end do
